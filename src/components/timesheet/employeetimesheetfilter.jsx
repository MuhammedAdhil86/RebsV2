      ...
      <div className="flex flex-wrap items-end gap-3">
        ...
        <div>
          ...
          <select
            ...
            className="w-60 border border-gray-300 rounded-lg px-3 py-1.5 text-xs font-normal focus:ring-1 focus:ring-black focus:outline-none bg-white"
          >
            <option value="all">All Employees</option>
            {employeeOptions.map((emp) => {
              // Value sent to API (prefers uuid, falls back to id)
              const empValue = emp.uuid || emp.id;
              ...
              const empCode = emp.uuid || emp.id;

              return (
                <option key={empValue} value={empValue}>
                  {fullName} ({empCode})
                </option>
              );
            })}
