...
const INITIAL_FORM = {
  ...
  subject: "Insurance Claim Confirmation - {{.CompanyName}}",
...
export default function CreateLetterEmailTemplateForm({
  onBack,
  onSuccess,
  ...
  const handleSubmit = async () => {
    if (
      ...
      toast.error("Template name, purpose, subject, and content are required");
      return;
    }
    ...
    try {
      ...
      toast.success(response?.message || "Email template created successfully");
      ...
      <div className="p-8 flex flex-col gap-6 max-h-[78vh] overflow-y-auto no-scrollbar bg-[#FAFBFC]">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <input
            type="text"
            name="subject"
            placeholder="e.g. Important Notice: Your Insurance Claim Confirmation"
            className="w-full px-5 py-3 bg-white border border-gray-200 rounded-xl text-[12px] focus:ring-1 focus:ring-black outline-none transition-all"
