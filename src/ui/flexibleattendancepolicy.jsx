...
const FlexibleUpdateAttendancePolicyTab = ({ initialData, onClose }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    is_flexible: false, // Ensure this is part of the state
  });

  // LOGIC: Check if field is editable
  // Only start_date and end_date are editable if is_flexible is true
  const isLocked = (field) => {
    if (formData.is_flexible) {
      return !["start_date", "end_date"].includes(field);
    }
    return false;
  };
  ...
  const handleInputChange = (field, value) => {
    if (isLocked(field)) return; // Block changes to locked fields
  ...
  const lockClass =
    "bg-gray-50 text-gray-400 cursor-not-allowed border-none shadow-none";
  const labelClass =
    ...
    return (
      <div className="flex flex-col gap-1.5">
        ...
        <button
          ...
          className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none ${value ? (locked ? "bg-gray-400" : "bg-black") : "bg-gray-300"} ${locked ? "cursor-not-allowed" : ""}`}
        >
          <span
      ...
      <div>
        ...
        <div className="relative group">
          <input
            ...
            className={`${inputClass} pr-10 ${locked ? lockClass : ""}`}
            onChange={(e) => handleInputChange(field, e.target.value)}
          />
          <div
            className={`${iconWrapper} ${locked ? "pointer-events-none" : ""}`}
            onClick={() => !locked && inputRef.current?.showPicker()}
          >
            <Clock size={14} />
          </div>
        </div>
      ...
      <div className="flex gap-6">
        ...
        <div className="w-[32%] space-y-4">
          <div>
            ...
            <input
              type="text"
              readOnly={isLocked("policy_name")}
              className={`${inputClass} ${isLocked("policy_name") ? lockClass : ""}`}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                type="text"
                readOnly={isLocked("policy_code")}
                className={`${inputClass} ${isLocked("policy_code") ? lockClass : ""}`}
          ...
          {formData.over_time_benefit && (
            <div className="animate-in fade-in slide-in-from-top-1">
              ...
              <input
                type="number"
                readOnly={isLocked("over_time_pay")}
                placeholder="0.00"
                className={`${inputClass} border-orange-100 bg-orange-50/30 font-semibold text-orange-700 ${isLocked("over_time_pay") ? "bg-gray-50 text-gray-400 border-none" : ""}`}
          ...
          {formData.consider_as_halfday && (
            <div className="animate-in fade-in slide-in-from-top-1">
              ...
              <div className="relative">
                <select
                  disabled={isLocked("policy_halfday_type")}
                  className={`${inputClass} border-blue-100 bg-blue-50/30 pr-10 appearance-none ${isLocked("policy_halfday_type") ? "bg-gray-50 text-gray-400 border-none" : ""}`}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                type="number"
                readOnly={isLocked("regularisation_limit")}
                className={`${inputClass} ${isLocked("regularisation_limit") ? lockClass : ""}`}
            ...
            <div className="relative">
              ...
              <select
                disabled={isLocked("regularisation_type")}
                className={`${inputClass} appearance-none ${isLocked("regularisation_type") ? lockClass : ""}`}
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="relative">
              <label className={labelClass}>Period</label>
              <select
                disabled={isLocked("overtime_cap_period")}
                className={`${inputClass} appearance-none ${isLocked("overtime_cap_period") ? lockClass : ""}`}
        ...
        <div className="w-[68%] space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            {/* Delay Section */}
            <div
              className={`bg-[#FFF6E9] border border-[#FDE3C3] rounded-2xl p-5 space-y-4 ${isLocked("delay_action_type") ? "opacity-75" : ""}`}
            >
              ...
              <div className="flex items-center gap-3">
                <input
                  type="number"
                  readOnly={isLocked("delay_unpaid_count")}
                  className={`w-20 bg-white h-10 rounded-xl px-3 text-[12px] shadow-sm border-none focus:outline-none ${isLocked("delay_unpaid_count") ? "cursor-not-allowed" : ""}`}
                ...
                <div className="relative flex-1">
                  <select
                    disabled={isLocked("delay_action_type")}
            ...
            {/* Late Section */}
            <div
              className={`bg-[#FFEBF3] border border-[#FFD2E5] rounded-2xl p-5 space-y-4 ${isLocked("late_action_type") ? "opacity-75" : ""}`}
            >
              <div className="flex justify-between items-center text-gray-800 text-[13px] font-medium">
                Consider Late{" "}
                <span className="text-gray-500 font-normal">Compensates</span>
              ...
              <div className="flex items-center gap-3">
                <input
                  type="number"
                  readOnly={isLocked("late_unpaid_count")}
                  className={`w-20 bg-white h-10 rounded-xl px-3 text-[12px] shadow-sm border-none focus:outline-none ${isLocked("late_unpaid_count") ? "cursor-not-allowed" : ""}`}
                ...
                <div className="relative flex-1">
                  <select
                    disabled={isLocked("late_action_type")}
                    className="w-full bg-white h-10 rounded-xl pl-3 pr-10 text-[12px] shadow-sm border-none appearance-none focus:outline-none"
...
export default FlexibleUpdateAttendancePolicyTab;
